...
| 组件 | 状态 | 安装命令 | 备注 |
|------|------|----------|------|
| MinerU | ⚠️ 已安装，需修复模型 | `pip install magic-pdf[full]` | 模型权重不匹配，需重新下载 |
...
**MinerU 模型问题说明**：
- 版本：magic-pdf 1.3.12
- 问题：模型权重大小不匹配（`size mismatch for backbone.blocks3.0...`）
- 原因：模型文件版本与代码版本不匹配
- 解决方案：需要重新下载正确的模型文件到 `D:\MinerU\models\PDF-Extract-Kit\models\`
