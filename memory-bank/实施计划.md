...
```json
{"id": "sec2.1", "type": "section", "title": "Method", "content": "...", "page": 3, "source_ref": "p3#sec2.1"}
```
...
- `academic-researcher` 与 `math-reasoning` 为 Claude Code skill。
- 脚本生成 clean_md + PageIndex 后交给 Claude。
- Claude 根据内容判断是否调用 skill。
- 无固定接口，由 Claude 决定调用时机。
...
| 组件 | 状态 | 安装命令 | 备注 |
|------|------|----------|------|
| MinerU | ⚠️ 已安装，OCR模型版本不匹配 | `pip install magic-pdf[full]` | v1.3.10，OCR模型v3/v5不匹配 |
| academic-researcher | ✅ 已安装 | `npx skills add shubhamsaboo/awesome-llm-apps@academic-researcher -g -y` | |
| math-reasoning | ✅ 已安装 | `npx skills add lingzhi227/agent-research-skills@math-reasoning -g -y` | |
| PyMuPDF | ✅ 可用 | `pip install PyMuPDF` | 替代方案，可直接提取文本型PDF |

**MinerU 问题说明**：
- 版本：magic-pdf 1.3.10
- 问题：OCR模型权重不匹配（`size mismatch for backbone.blocks3.0...`）
- 原因：下载的模型是v5版本，但代码期望v3版本
- 影响：`auto`、`txt`、`ocr`模式均失败
- **替代方案**：使用PyMuPDF（fitz）直接提取文本型PDF

**V1策略调整**：
- 文本型PDF：优先使用PyMuPDF提取
- 图片型PDF：需要MinerU OCR时，等待模型版本修复
- 后续：寻找MinerU官方模型下载工具或使用正确版本的模型
