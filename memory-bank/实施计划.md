...
6. prepare_summary_input.py
   - 读取 clean_md 和 index.meta.json，提取章节结构和关键上下文。
   - 生成 `outputs/<stem>/summary/input-for-summary.md` 供 Claude 调用 skill。
   - **验证**：生成的输入文件包含章节结构和关键段落

7. prepare_formula_input.py
   - 从 clean_md 中提取公式块和上下文段落。
   - 生成 `outputs/<stem>/summary/input-for-formula.md` 供 Claude 调用 skill。
   - **验证**：对公式密集型 PDF，生成的输入文件包含公式块

8. write_failure_report.py
   - 记录失败原因与可重试建议。
   - **验证**：模拟失败场景后，`failure.md` 包含错误原因和建议

**里程碑 M2**：单篇 PDF 可完整提取并生成 clean_md、PageIndex 和数据准备文件
- **验证**：运行 `python scripts/run_workflow.py --file projects/02-Imaginary-time-Mpemba-effect.pdf` 成功
- **验证**：`outputs/02-Imaginary-time-Mpemba-effect/` 目录包含 `raw_md/`、`clean_md/`、`pageindex/`、`summary/`
- **验证**：`交付指引.md` 已生成，包含文件清单和执行指令

---

### 阶段3：Skill 调用与结果生成（Claude 参与）

本阶段由 Claude 手动执行，脚本已在阶段2完成数据准备。

**执行流程**：
1. Claude 读取 `outputs/<stem>/交付指引.md` 获取文件清单
2. 读取 `summary/input-for-summary.md`，调用 `academic-researcher` skill 生成 one-pager.md
3. 读取 `summary/input-for-formula.md`（如存在），调用 `math-reasoning` skill 生成 formula-notes.md
4. 基于 clean_md 生成 section-notes.md、glossary.md、followups.md

**里程碑 M3**：完整输出结构可用
- **验证**（Claude 手动）：`outputs/<stem>/summary/` 包含 `one-pager.md`、`section-notes.md`、`glossary.md`、`followups.md`
- **验证**（Claude 手动）：`one-pager.md` 每个模块不少于 2 句话
- **验证**（Claude 手动）：`formula-notes.md` 包含推导步骤（公式密集型 PDF）
...
| M2 | 单篇 PDF 完整提取并生成 clean_md、PageIndex 和数据准备文件 | ⬜ |
| M2 | `交付指引.md` 已生成，包含文件清单和执行指令 | ⬜ |
...
### 12.4 自动生成文件
- outputs/<stem>/交付指引.md（由 run_workflow.py 自动生成）
...
| 当前状态 | ✅ 已安装并验证（magic-pdf 1.3.12，详见18.6节） |
...
| 7 | MinerU安装状态 | ✅ 已安装并验证（magic-pdf 1.3.12，详见18.6节） |
| 8 | 批量处理方式 | V1顺序处理 |
| 9 | 日志格式 | 见18.8 |
| 10 | CLI接口 | 见18.9 |
| 11 | Skill调用方式 | 脚本准备数据 + Claude手动调用skill |
| 12 | 交付指引.md生成 | run_workflow.py 流程结束后自动生成 |
| 13 | 阶段3验证方式 | Claude手动验证skill调用结果 |
